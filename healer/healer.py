...
def create_dummy_entry_done():
    ...
    fen = {"_id" : UUID, "fen" : FEN, "status" : "done", "lastqueued" : datetime.datetime.utcnow() - datetime.timedelta(seconds=60)}
    db.fens.insert_one(fen)
    r.rpush(DB_NAME, UUID)
...
def create_dummy_entry_processing():
    ...
    fen = {"_id" : UUID, "fen" : FEN, "status" : "processing", "lastqueued" : datetime.datetime.utcnow() - datetime.timedelta(seconds=60)}
    db.fens.insert_one(fen)
    r.rpush(DB_NAME, UUID)
...
def searchQueue(uuid):
  if r.llen("work") == 0: return False
  inQueue = False
...
def oldLoop():
  ...
  for item in query:
    ...
    if (not searchQueue(item["_id"])):
      uuid=db.fens.update({"_id":item["_id"]},item)
      # push to queue
      r.rpush(DB_NAME, item["_id"])
      print("Updated item:", item)
    else: print("Passed item because it was still in the queue:", item)
  time.sleep(5)
  print("Loop finished; Time Elapsed = ", datetime.datetime.utcnow() - starttime)

while True:
  thirtysecs = datetime.datetime.utcnow() - datetime.timedelta(seconds=30) # Store the time 30 seconds ago in a variable to be used for checking 
  query = db.fens.find({"lastqueued": {"$lt": thirtysecs}, "status" : "processing"}) # Query the database to find items older than 30 seconds old and with a status of "processing" (i.e. they started getting processing but did not complete within 30 sceonds, something likely crashed)
  for item in query: # for each of these items
    item["lastqueued"] = datetime.datetime.utcnow() # update the "lastqueued" tag to the current time
    item["status"] = "pending"

    uuid=db.fens.update({"_id":item["_id"]},item) # update that item in the mongodb database
  ...
  print("Loop finished; Time Elapsed =", datetime.datetime.utcnow() - starttime) # print time elapsed for keeping track